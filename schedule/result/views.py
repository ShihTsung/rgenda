...
def get_workday_num(user_id, cycle_start, cycle_end, start=None, end=None):
    """
    取得指定員工在起訖日內的值班天數
    若開始日期(每月一號)不是週期的第一天，則將員工該週期開始日前已排定的上班日數(同週期內上個月的班表)扣除
    :param user_id:
    :param cycle_start: 週期的起始日
    :param cycle_end: 週期的結束日
    :param start: 開始日
    :param end: 結束日
    :return:
    """
...
def check_eoa(user, date):
    try:
        last1 = Result.objects.get(user=user, date=date - timedelta(days=1))
        last2 = Result.objects.get(user=user, date=date - timedelta(days=2))
    except Result.DoesNotExist:
        return 0

    if last1.shift.shift_type in [1, 2]:
        return 2
    elif last2.shift.shift_type in [1, 2]:
        return 1
    return 0
        ...
        for shift in shifts:
            ...
            for demand in demands:
                ...
                for ind, cycle in enumerate(cycle_list):
                    ...
                    attr_dict[ind]['total_demands'] = total_demands
                    attr_dict[ind]['q'] = diff_q
                    attr_dict[ind]['r'] = diff_r
                ...
                for ind, cycle in enumerate(cycle_list):
                    ...
                    for _ in range(10000):
                        ...
                        demand_loss = attr_dict[ind]['total_demands']
