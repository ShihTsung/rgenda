    ...
    <div class="col-6">
      <div class="card mx-1">
        ...
        <div class="card-body DashCollapse" v-show="show">
          ...
          <div v-else class="mx-auto h5 no-data">無資料</div>
        </div>
      </div>
    </div>
...
import { shiftColor } from "src/constants/color.js";
...
export default {
  ...
  methods: {
      ...
      this.results.forEach((result) => {
        if (this.$isWorkShift(result.shift.shift_type)) {
          ...
          if (
            ...
            this.$getShiftTypeValue("VALUE_ON_CALL") == result.shift.shift_type
          ) {
            shiftStat.onCall++;
          }
        } else {
          ...
          if (leaveItem) {
            if (
              this.$getPromiseLeaveItemValue("ITEM_REST_LEAVE") === leaveItem.id
            ) {
              dates.restLeave.number++;
            } else if (
              this.$getPromiseLeaveItemValue("ITEM_OFFICIAL_HOLIDAY") ===
              leaveItem.id
            ) {
              dates.officialHoliday.number++;
            } else if (
              this.$getPromiseLeaveItemValue("ITEM_NATIONAL_HOLIDAY") ===
              leaveItem.id
            ) {
              dates.nationalHoliday.number++;
            } else if (
              this.$getPromiseLeaveItemValue("ITEM_ANNUAL_LEAVE") ===
              leaveItem.id
            ) {
            ...
            if (
              this.$getPromiseLeaveItemValue("ITEM_EMPTY_SHIFT") ===
              leaveItem.id
            ) {
    ...
    getHoursData() {
      ...
      this.results.forEach((result) => {
        let isSameOrBefore = moment(result.date).isSameOrBefore(
          moment(),
          "day"
        );
        ...
        if (this.$isWorkShift(result.shift.shift_type)) {
          ...
          if (
            this.$getShiftTypeValue("VALUE_OFFICIAL_LEAVE") ===
            result.shift.shift_type
          ) {
      ...
      this.timeAdjustments.forEach((adjustment) => {
        if (adjustment.user === parseInt(this.userId)) {
          let isSameOrBefore = moment(adjustment.date).isSameOrBefore(
            moment(),
            "day"
          );
          ...
          if (adjustment.adjustment_type === 0) {
            ...
            if (isSameOrBefore) {
              if (
                ...
                this.$getTimeAdjustmentItemValue("ITEM_EMPLOYEE_LEAVE") ===
                adjustment.adjustment_item
              ) {
      ...
      let offDays = this.results.filter((result) => {
        return (
          !this.$isWorkShift(result.shift.shift_type) &&
          this.$getShiftTypeValue("VALUE_OFFICIAL_LEAVE") !==
            result.shift.shift_type
        );
      }).length;
      // 休息日出勤時數
      let offDateAttendantHours = this.timeAdjustments.reduce(
        (accumulator, currentValue) => {
          if (
            currentValue.user === parseInt(this.userId) &&
            this.$getTimeAdjustmentItemValue("ITEM_OFF_DAY_ATTENDANCE") ===
              currentValue.adjustment_item
          ) {
