  ...
  <div>
      ...
      <template slot="table-row" slot-scope="props">
        <span v-if="props.column.field === 'content'" class="text-nowrap row py-2">
          <span class="col-5">
            <span
              class="align-middle"
            >{{ props.row.user_apply ? props.row.apply_result.user.name : props.row.receive_result.user.name }}</span>
          ...
          <span class="col-5">
            ...
            <span
              class="align-middle"
              v-if="props.row.user_apply"
            >{{ props.row.receive_result.user.name }}</span>
          </span>
        </span>
          ...
          >
            ...
            <div class="row">
              <div class="offset-2 col-8">
                <p
                  class="mb-4"
                >日期：{{ rows[approveIdx].date_start }} ~ {{ rows[approveIdx].date_end }}</p>
                <div class="row mb-2">
                  <div class="col-4">
    ...
    <div class="modal fade" id="withdrawModal" tabindex="-1" role="dialog" aria-hidden="true">
      <div class="modal-dialog modal-dialog-centered modal-dialog-scrollable">
        <div class="modal-content">
          <div class="modal-header border-bottom-0">
            <button type="button" class="close" aria-label="Close" @click="cancelWithdraw()">
              <span aria-hidden="true">
                <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24">
                  <path
                    d="M24 20.188l-8.315-8.209 8.2-8.282-3.697-3.697-8.212 8.318-8.31-8.203-3.666 3.666 8.321 8.24-8.206 8.313 3.666 3.666 8.237-8.318 8.285 8.203z"
          ...
          >
            ...
            <div class="row">
              <div class="offset-2 col-8">
                ...
                <p
                  class="mb-4"
                >日期：{{ rows[withdrawIdx].date_start }} ~ {{ rows[withdrawIdx].date_end }}</p>
                <p class="mb-2">
                  原排班別：
                  <span class="shift-cell" :style="rows[withdrawIdx].apply_result.style">
                    <span class="h5 shift-type">{{ rows[withdrawIdx].apply_result.shift.name }}</span>
                    <span
                      class="shift-time"
                    >{{ getShiftTimeStr(rows[withdrawIdx].apply_result.shift) }}</span>
                    <span class="shift-station">{{ rows[withdrawIdx].apply_result.station.name }}</span>
                  </span>
                </p>
                <p class="mb-2">
                  欲換班別：
                  <span class="shift-cell" :style="rows[withdrawIdx].receive_result.style">
                    <span class="h5 shift-type">{{ rows[withdrawIdx].receive_result.shift.name }}</span>
                    <span
                      class="shift-time"
                    >{{ getShiftTimeStr(rows[withdrawIdx].receive_result.shift) }}</span>
                    <span class="shift-station">{{ rows[withdrawIdx].receive_result.station.name }}</span>
                  </span>
                </p>
                <p class="mb-4">換班人員：{{ rows[withdrawIdx].receive_result.user.name }}</p>
                <div class="row">
                  <div class="col mb-2">
                    <button type="button" class="btn btn-rgenda" @click="cancelWithdraw()">取消</button>
                    <button class="btn btn-rgenda" type="button" @click="confirmWithdraw()">確定</button>
                  </div>
                </div>
              </div>
            </div>
